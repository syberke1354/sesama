...
<div class="container-fluid">
    ...
    <h2 class="fw-bold mb-4">Edit Program</h2>
        ...
        <div class="card-body p-4">
            <form action="{{ route('admin.programs.update', $program['id']) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" required>{{ old('description', $program['description']) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="full_description" class="form-control @error('full_description') is-invalid @enderror" rows="8" required>{{ old('full_description', $program['full_description']) }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <input type="url" name="image_url" class="form-control @error('image_url') is-invalid @enderror" value="{{ old('image_url', $program['image_url']) }}">
                    @error('image_url')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="category" class="form-select @error('category') is-invalid @enderror" required>
                            <option value="health" {{ old('category', $program['category']) === 'health' ? 'selected' : '' }}>Kesehatan</option>
                            <option value="education" {{ old('category', $program['category']) === 'education' ? 'selected' : '' }}>Pendidikan</option>
                            <option value="social" {{ old('category', $program['category']) === 'social' ? 'selected' : '' }}>Sosial</option>
                            <option value="general" {{ old('category', $program['category']) === 'general' ? 'selected' : '' }}>Umum</option>
                        </select>
                        @error('category')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
